...
g_pcvar_mybox_spawn_onstart, g_pcvar_mybox_loop_spawn_time, g_pcvar_mybox_teddybear_chance,
g_FW_FM_ADDTOFULLPACK_PRE, g_fw_mybox_open_attempt;
...
public plugin_init()
{
	...
	g_pcvar_mybox_teddybear_chance = register_cvar("mybox_teddybear_chance", "30");
...
spawn_mysterybox_decoy(const district)
{
	...
	set_pev(box, pev_flags, FL_ALWAYSTHINK);
}
...
spawn_mysterybox(const district)
{
	...
	for(new i; i < g_total_items; i++)
	{
		...
		maxchances += xArray[ITEM_CHANCES];
	}

	maxchances += get_pcvar_num(g_pcvar_mybox_teddybear_chance) * g_total_items;

	for(new itemX, x, f; itemX < g_total_items; itemX++)
	{
		f = clamp( x + floatround( 100.0 * (float(ArrayGetCell(tArray, itemX)) / float(maxchances)) ), 0, 100 );
		arrayset(chances[x], itemX, f - x);
		x = min(f, sizeof chances);
	}
...
show_mysterybox_spawners( )
{
	...
	if( !g_FW_FM_ADDTOFULLPACK_PRE )
	{
		g_FW_FM_ADDTOFULLPACK_PRE = register_forward(FM_AddToFullPack, "fw_AddToFullPack_pre", ._post=false);
	}
}
...
{
	if(g_FW_FM_ADDTOFULLPACK_PRE && !user_editor_mode)
	{
		unregister_forward(FM_AddToFullPack, g_FW_FM_ADDTOFULLPACK_PRE, .post=false);
		g_FW_FM_ADDTOFULLPACK_PRE = 0;
