...
g_pCvarMaxRange, g_pCvarMinRange;
...
public task_show_seeker_radar(taskid)
{
    ...
    const MAXChars = 20;
    new Float:MAXfRadius = get_pcvar_float(g_pCvarMaxRange);
    new Float:MINfRadius = get_pcvar_float(g_pCvarMinRange);
    ...
    iCharged = floatround((floatclamp((MAXfRadius - fMinDistance) * (MAXfRadius / MINfRadius), 0.0, MAXfRadius) / MAXfRadius) * MAXChars);
...
new bool:g_bReGameDLL = false, g_nVault_npcs_spawns, g_szMapName[MAX_MAPNAME_LENGTH];
...
public plugin_init()
{
    ...
    g_pCvarMaxRange = register_cvar("jb_sd_gw_hidermeter_maxrange", "1400.0");
    g_pCvarMinRange = register_cvar("jb_sd_gw_hidermeter_minrange", "700.0");
    ...
    g_nVault_npcs_spawns = nvault_open("guesswho_npcs_spawns");

    get_mapname(g_szMapName, charsmax(g_szMapName));

    new szKey[MAX_MAPNAME_LENGTH + 16]
    formatex(szKey, charsmax(szKey), "%s_NPCS_MAX_SPAWNS", g_szMapName);
    new iNPCsSpawns = nvault_get(g_nVault_npcs_spawns, szKey);

    if(iNPCsSpawns > 0)
    {
        FoundOriginsArray = ArrayCreate(3);
        LoadSpawns(g_nVault_npcs_spawns, iNPCsSpawns);
    }
    else
    {
        ROGInitialize(250.0, "rog_check_origin");
    }

    nvault_close(g_nVault_npcs_spawns);
...
public rog_check_origin(Float:fOrigin[3])
{
    if(g_nVault_npcs_spawns != INVALID_HANDLE)
    {
        ...
        nvault_set(g_nVault_npcs_spawns, szKey, szValue);
