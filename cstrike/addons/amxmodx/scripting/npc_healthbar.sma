...
new g_msg_SayText, g_fw_AddToFullPackPre = INVALID_HANDLE, Trie:g_Trie_RegisteredNPCs = Invalid_Trie;
...
public plugin_init( ) 
{
	...
	g_Trie_RegisteredNPCs = TrieCreate();

	new fp = fopen(szFile, "a+"), szClass[ 32 ], szOffset[ 3 ];
	...
	if( fp > 0 )
	{
		while(fgets(fp, szBuffer, charsmax(szBuffer)))
		{
			...
			argbreak(szBuffer, szClass, charsmax(szClass), szOffset, charsmax(szOffset));
			NPC_Hook_Event(szClass, NPC_EVENT_TAKEDAMAGE, "fwNPCTakeDamage", iPluginID);
			NPC_Hook_Event(szClass, NPC_EVENT_DEATH, "fwNPCDeath", iPluginID);
			TrieSetCell(g_Trie_RegisteredNPCs, szClass, floatstr(szOffset));
		}
		fclose(fp);
	}
...
public plugin_end()
{
	TrieDestroy(g_Trie_RegisteredNPCs);
}
...
CreateHealthBar(const pOwner)
{
	...
	if( pev_valid( ent ) )
	{
		...
		new szClass[ 32 ], Float:fNPCOrigin[ 3 ], Float:fViewOFS[ 3 ], Float:fOffset_Z;
		pev( pOwner, pev_origin, fNPCOrigin );
		pev( pOwner, pev_view_ofs, fViewOFS );
		pev( pOwner, pev_classname, szClass, charsmax(szClass) );
		TrieGetCell(g_Trie_RegisteredNPCs, szClass, fOffset_Z);
		fNPCOrigin[ 2 ] = fNPCOrigin[ 2 ] + fViewOFS[ 2 ] + fOffset_Z;
...
public fwAddToFullPack( es, e, ent, host, host_flags, player, p_set )
{
	if( !player && !g_playerBot[ host ] && g_isAlive[ host ] )
	{
		...
		{	
			...
			static szClass[ 32 ], Float:fNPCOrigin[ 3 ], Float:fViewOFS[ 3 ], Float:fOffset_Z;
			pev( npc, pev_classname, szClass, charsmax(szClass) );
