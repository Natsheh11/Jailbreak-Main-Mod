...
#define NPC_REMOTE_CONTROL 907050
...
RemovePlayerTurrets(id)
{
    new ent, iTargetFlags, iOwner;
    while( (ent = find_ent_by_owner(ent, TURRET_CLASSNAME, id)) > 0)
    {
        iOwner = pev(ent, PEV_OWNER);

        if((iTargetFlags=pev(ent, PEV_TARGET_FLAGS)) & TURRET_REMOTE_CONTROLLED)
        {
            ...
            RemoveTurretCamera(ent);
            set_pev(ent, PEV_TARGET_FLAGS, iTargetFlags & ~TURRET_REMOTE_CONTROLLED);
            set_pev(ent, PEV_TASK, STATE_TURRET_ACTIVE);
        }
...
turret_control_menu(const id, const turret)
{
    ...
    {
        menu_additem(menu, "Remote controlling!", szInfo, 0);
    }
...
public turret_control_m_handle(id, menu, item)
{
    ...
    switch( item )
    {
        ...
        case 6:
        {
            if(!(iTargetFlags & TURRET_REMOTE_CONTROLLED))
            {
                set_pev(turret, PEV_TASK, NPC_REMOTE_CONTROL);
                set_pev(turret, PEV_TARGET_FLAGS, (iTargetFlags | TURRET_REMOTE_CONTROLLED));
                create_3rd_camera(turret, id);
                dllfunc(DLLFunc_Think, turret);
            }
            else
            {
                attach_view(id, id);
                set_pev(turret, PEV_TASK, STATE_TURRET_ACTIVE);
                set_pev(turret, PEV_TARGET_FLAGS, (iTargetFlags & ~TURRET_REMOTE_CONTROLLED));
                dllfunc(DLLFunc_Think, turret);

                RemoveTurretCamera(turret);
            }
...
RemoveTurretCamera(const turret)
{
    new iOwner;
    if(is_user_connected((iOwner=pev(turret, PEV_OWNER))))
    {
        ExecuteHamB(Ham_CS_Player_ResetMaxSpeed, iOwner);
    }

    new ent;
    while( (ent = find_ent_by_tname(ent, "turret_camera")) > 0)
    {
        if(pev(ent, pev_owner) != turret) continue;
...
public camera_think(ent)
{
    ...
    static szClass[sizeof TURRET_CLASSNAME];
    pev(iNPC, pev_classname, szClass, charsmax(szClass));

    if(!equal(szClass, TURRET_CLASSNAME))
    {
        return;
    }
    ...
    GetTurretGunAngles(iNPC, fAngle);
    fAngle[0] *= -1.0;
...
create_3rd_camera(iOwner, iduser)
{
    ...
    set_pev(iEnt, pev_targetname, "turret_camera");
...
public turret_brain(id)
{
    ...
    switch( iState )
    {
        case NPC_REMOTE_CONTROL:
        {
            if(pev(iOwner, pev_button) & IN_USE)
            {
                turret_control_menu(iOwner, id);
            }

            static xArray[ANIMATION_DATA], Float:fTargetAngles[3];

            pev(iOwner, pev_v_angle, fTargetAngles);
            fTargetAngles[0] *= -1.0;
            FixAngles(fTargetAngles);
            ...
            if(get_user_button(iOwner) & IN_ATTACK)
            {
        ...
        case NPC_DEATH:
        {
            if(pev(id, PEV_TARGET_FLAGS) & TURRET_REMOTE_CONTROLLED)
            {
                attach_view(iOwner, iOwner);
                RemoveTurretCamera( id );
            }
        ...
        case STATE_TARGET_ENGAGE:
        {
            ...
            if(fHealth > 0.0)
            {
                ...
                /* static Float:fVec1[3], Float:fVec2[3];
                xs_vec_copy(fTraceAttackOrigin, fVec1);
                xs_vec_mul_scalar(fDirection, 50.0, fVec2);
                xs_vec_add(fTraceAttackOrigin, fVec2, fVec2);
                create_laserbeam(fVec1, fVec2);

                xs_vec_mul_scalar(fNPCVecDirection, 50.0, fVec2);
                fVec2[2] *= -1.0;
                xs_vec_add(fTraceAttackOrigin, fVec2, fVec2);
                create_laserbeam(fVec1, fVec2, .green = 0, .red = 255);*/
...
stock create_laserbeam(Float:fVecStart[3], Float:fVecEnd[3], red=0, green=255, blue=0, brightness=200, width=5)
{
    message_begin(MSG_BROADCAST, SVC_TEMPENTITY);
    write_byte(TE_BEAMPOINTS)
    engfunc(EngFunc_WriteCoord, (fVecStart[0])) // End X
    engfunc(EngFunc_WriteCoord, (fVecStart[1])) // End Y
    engfunc(EngFunc_WriteCoord, (fVecStart[2])) // End Z
    engfunc(EngFunc_WriteCoord, (fVecEnd[0])) // End X
    engfunc(EngFunc_WriteCoord, (fVecEnd[1])) // End Y
    engfunc(EngFunc_WriteCoord, (fVecEnd[2])) // End Z
...
GetTurretGunAngles(id, Float:fAngles[3])
{
    ...
    fAngles[1] += (fYaw / 255.0) * (floatabs(fMaxYaw) + floatabs(fMinYaw));
...
SetTurretGunAngles(id, Float:fAngles[3])
{
    ...
    static Float:fPrevAngles[3], Float:fTurretAngles[3];
    GetTurretGunAngles(id, fPrevAngles);
    pev(id, pev_angles, fTurretAngles);

    if(fAngles[0] >= 180.0) fAngles[0] -= 360.0;
    if(fPrevAngles[0] >= 180.0) fPrevAngles[0] -= 360.0;

    iDiff = floatround((fAngles[0] - fPrevAngles[0]) * (floatcos(fTurretAngles[0],degrees) / floatabs(floatcos(fTurretAngles[0],degrees))));
    fPitch = float( iDiff % floatround((floatabs(fMaxPitch) + floatabs(fMinPitch))) );

    if(iDiff != 0 && is_angle_between(fAngles[0], fMinPitch, fMaxPitch))
    {
        if(g_aBoneControllersData[iType][1][BONE_CONTROLLER_REVERSED])
        {
            if(iDiff > 0)
    ...
    iDiff = floatround((fAngles[1] - fPrevAngles[1]) * (floatcos(fTurretAngles[0],degrees) / floatabs(floatcos(fTurretAngles[0],degrees))));
    fYaw = float( iDiff % floatround((floatabs(fMaxYaw) + floatabs(fMinYaw))) );

    if(iDiff != 0 && (fMinYaw <= fAngles[1] <= fMaxYaw))
    {
        if(iDiff > 0)
            fYaw = fYawController + (fYaw / (floatabs(fMaxYaw) + floatabs(fMinYaw))) * 255.0;
        else
            fYaw = fYawController + ((fYaw / (floatabs(fMaxYaw) + floatabs(fMinYaw))) - 1.0) * 255.0;
        set_pev(id, BONE_CONTROL_ROTATION_YAW, floatround(fYaw) % 255);
    }
}

bool:is_angle_between(Float:angle, Float:min_pitch, Float:max_pitch)
{
    if(angle >= 360.0) angle -= 360.0;
    else if(angle < 0.0) angle += 360.0;
    if(min_pitch >= 360.0) min_pitch -= 360.0;
    else if(min_pitch < 0.0) min_pitch += 360.0;
    if(max_pitch >= 360.0) max_pitch -= 360.0;
    else if(max_pitch < 0.0) max_pitch += 360.0;

    if (min_pitch <= max_pitch)
        return (min_pitch <= angle <= max_pitch);

    return (angle >= min_pitch) || (angle <= max_pitch);
}
