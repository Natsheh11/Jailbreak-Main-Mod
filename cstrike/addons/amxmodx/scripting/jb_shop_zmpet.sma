...
public plugin_init()
{
	...
	g_DAY_ID = register_jailbreak_day("Zombie Apocalypse", 0, 500.0, DAY_TIMER);
...
NPC_DestroyAttachments( const iNPC )
{
	...
	while( (ent = find_ent_in_sphere(ent, fOrigin, xs_vec_len(fVec) + 100.0)) > 0 )
	{
	...
	switch( iTask )
	{
		...
		case NPC_MANUAL_CONTROL:
		{
			if(!iCameraEntityIndex)
			{
				...
				iCameraEntityIndex = create_3rd_camera(id, iOwner);
