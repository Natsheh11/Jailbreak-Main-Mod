        ...
        - - - - - - - - - -
            * TerminateRound( const roundEndType, const teamWinning = TeamWinning_None, const mapType = MapType_AutoDetect );
                ...
                // Time is up / round draw.
                    ...
                    TerminateRound( TR_RoundEndType_Timer );
                ...
                // The current map objective of the team has been completed.
                    ...
                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Terrorist );
                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Ct );

                // The provided map objective of the team has been completed. (will work only if the related entity is present)

                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Terrorist, MapType_VipAssasination );
                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Terrorist, MapType_Hostage );
                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Terrorist, MapType_PrisonEscape );
                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Ct, MapType_VipAssasination );
                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Ct, MapType_Bomb );
                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Ct, MapType_Hostage );
                    TerminateRound( TR_RoundEndType_Objective, TeamWinning_Ct, MapType_PrisonEscape );
...
#define REGAMEDLL
    ...
    /* MACROS */
        ...
        new const g_szGameRulesMI[ eGameRulesMembers ][] =
        {
            "m_bRoundTerminating",
            "m_iRoundWinStatus",
            "m_iRoundTimeSecs",
        ...
        #if !defined set_mp_pdata
        #define set_mp_pdata(%1,%2) ( OrpheuMemorySetAtAddress( g_pGameRules, g_szGameRulesMI[ %1 ], 1, %2 ) )
        #endif

        #if !defined get_mp_pdata
        #define get_mp_pdata(%1) ( OrpheuMemoryGetAtAddress( g_pGameRules, g_szGameRulesMI[ %1 ] ) )
        #endif
    ...
    public plugin_natives ()
    {
        ...
        register_native( "CheckWinConditions", "Native_CheckWinConditions" );
    }

    public Native_CheckWinConditions ( const plugin, const params )
    {
        _CheckWinConditions( .notifyAllPlugins = true );
        return 0;
    }
    ...
    public Native_TerminateRound ( const plugin, const params )
    {
        enum (+=1)
        {
            Param_RoundEndType = 1,
            Param_TeamWinning,
            Param_MapType
        }

        _TerminateRound( TR_RoundEndType:get_param( Param_RoundEndType ), get_param( Param_TeamWinning ), get_param( Param_MapType ) );
    }
    ...
    public ClientCommand_TerminateRound ( const player, const level, const cid )
    {
        ...
        new TR_RoundEndType:roundEndType;
        new teamWinning;
        new mapType;
        ...
        switch ( argumentRoundEndType[ 0 ] )
        {
            case 't', 'T' /* [t]imer/[t]eam */ :
            {
                ...
                {
                    case 'i', 'I' /* t[i]mer */ : { roundEndType = TR_RoundEndType_Timer; }
                    case 'e', 'E' /* t[e]am  */ : { roundEndType = TR_RoundEndType_TeamExterminate; needMoreArguments = true; }
                }
            }
            case 'o', 'O' /* [o]bjective */ : { roundEndType = TR_RoundEndType_Objective; needMoreArguments = true; }
            case 'd', 'D' /* [d]raw      */ : { roundEndType = TR_RoundEndType_Draw; }
        }
     ...
     */
    bool:_TerminateRound ( const TR_RoundEndType:roundEndType, const teamWinning, const mapType )
    {
        // Round is being terminated.
        if( get_mp_pdata( m_iRoundWinStatus ) )
        {
            return true;
        }
        ...
        switch ( roundEndType )
        {
...
#if defined REGAMEDLL
                set_mp_pdata( m_flRestartRoundTime    , 0.0 );
#elseif !defined REGAMEDLL
                set_mp_pdata( m_fRoundCount           , 0.0 );
#endif
            }
            case TR_RoundEndType_TeamExterminate :
            {
                ...
                /*
                    ...
                    | Triggered by default from CHalfLifeMultiplay::_CheckWinConditions().
                    |
                ...
                */
                ...
                if ( teamWinning == TeamWinning_Ct )
                {
                    ...
                    */
                    ...
                    {
                        if ( is_valid_ent( grenade ) && get_pdata_bool( grenade, m_bIsC4, linuxDiff ) && get_pdata_bool( grenade, m_bJustBlew, linuxDiff ) )
                        {
                            return false;
                        }
                    }
                ...
                new any:accountOffset;
                new any:numTeamOffset;
                ...
                switch ( teamWinning )
                {
                    ...
                    {
                        accountOffset = m_iAccountTerrorist;
                        numTeamOffset = m_iNumTerroristWins;
                    ...
                    case TeamWinning_Ct :
                    {
                        accountOffset = m_iAccountCT;
                        numTeamOffset = m_iNumCTWins;
                ...
                set_mp_pdata( accountOffset, get_mp_pdata( accountOffset ) + teamMoney );

                if ( get_mp_pdata( m_iNumSpawnableTerrorist ) && get_mp_pdata( m_iNumSpawnableCT ) )
                {
                    set_mp_pdata( numTeamOffset, get_mp_pdata( numTeamOffset ) + 1 );
                ...
                #if defined REGAMEDLL
                rg_round_end(.tmDelay=5.0, .st=winStatus, .trigger = true);
                #else
            ...
            case TR_RoundEndType_Objective :
            {
                /*
                            ...
                            */
                            ...
                            switch ( teamWinning )
                            {
                                ...
                                case TeamWinning_Ct :
                                {
                                    ...
                                    new Float:escapeRatio = get_mp_pdata( m_iHaveEscaped ) / floatmax( float( numEscapers ), 1.0 );
            ...
            case TR_RoundEndType_Draw :
            {
                ...
                */
                ...
                #if defined REGAMEDLL
                rg_round_end(.tmDelay=5.0, .st=WINSTATUS_DRAW, .event=ROUND_END_DRAW, .trigger = true);
                #else
    ...
    {
        static messageSendAudio=0;

        if ( messageSendAudio || ( messageSendAudio = get_user_msgid( "SendAudio" ) ) )
        {
            if ( notifyAllPlugins )
            {
                emessage_begin( MSG_ALL, messageSendAudio );
            ...
            {
                message_begin( MSG_ALL, messageSendAudio );
                write_byte( senderID );
    ...
    /**
     *  CHalfLifeMultiply::_CheckWinConditions().
     *  Check the win conditions and handle the round end.
     *
     *  @param notifyAllPlugins      If it should notify all the plugins. (hookable)
     */
    _CheckWinConditions ( const bool:notifyAllPlugins = false )
    {
#if defined REGAMEDLL
        #pragma unused notifyAllPlugins
        rg_check_win_conditions();
#else
        static OrpheuFunction:handleFunc_CheckWinConditions;

        if ( !handleFunc_CheckWinConditions )
        {
            handleFunc_CheckWinConditions = OrpheuGetFunctionFromObject( g_pGameRules, "CheckWinConditions", "CHalfLifeMultiplay" )
        }

        ( notifyAllPlugins ) ?
            OrpheuCallSuper( handleFunc_CheckWinConditions, g_pGameRules ) :
            OrpheuCall( handleFunc_CheckWinConditions, g_pGameRules );
#endif
    }
