...
new g_HARPOON_DAYID, g_HARPOON_ITEMID, g_HARPOON_LRITEMID, g_USER_HAS_HARPOON[33], g_pSpriteLightning,
...
public concmd_harpoon_storm(id, level, cid)
{
	...
	new Float:fOrigin[3], aParams[5], Array:ArrLocations = Invalid_Array;
	pev(id, pev_origin, fOrigin);
	find_sky(fOrigin, id);
	fOrigin[2] -= 100.0;
	aParams[1] = find_location_around_origin(fOrigin, Float:{ 16.0, 16.0, 1.0 }, Float:{ -16.0, -16.0, -1.0 }, 800.0, .pArrayLocations = ArrLocations);
	...
	task_harpoon_storm(aParams, id);
	return PLUGIN_HANDLED;
}

public task_harpoon_storm(aParams[5], const id)
{
    new Float:fOrigin[3], Array:ArrLocations = any:aParams[ 0 ];
    ...
    if(--aParams[2] > 0)
    {
        set_task(Float:aParams[4], "task_harpoon_storm", id, aParams, sizeof aParams);
    }
...
find_sky(Float:fOrigin[3], const ignore_ent=-1)
{
